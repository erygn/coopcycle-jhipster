<template>
  <div>
    <h2 id="page-heading" data-cy="BasketHeading">
      <span v-text="$t('coopcycleApp.basket.home.title')" id="basket-heading">Baskets</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('coopcycleApp.basket.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'BasketCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('coopcycleApp.basket.home.createLabel')"> Create a new Basket </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && baskets && baskets.length === 0">
      <span v-text="$t('coopcycleApp.basket.home.notFound')">No baskets found</span>
    </div>
      ...
      <table class="table table-striped" aria-describedby="baskets">
        <thead>
          <tr>
            ...
            <th scope="row"><span v-text="$t('coopcycleApp.basket.userCoop')">User Coop</span></th>
            <th scope="row"></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="basket in baskets" :key="basket.id" data-cy="entityTable">
            ...
            <td>
              <div v-if="basket.userCoop">
                <router-link :to="{ name: 'UserCoopView', params: { userCoopId: basket.userCoop.id } }">{{
                  basket.userCoop.id
                }}</router-link>
              </div>
            </td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="coopcycleApp.basket.delete.question" data-cy="basketDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-basket-heading" v-text="$t('coopcycleApp.basket.delete.question', { id: removeId })">
